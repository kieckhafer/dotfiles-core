...
- **Pre-commit or pre-push hook rejects a change citing leakage** → the check is intentional and withholds matched content by design. Compare the referenced file/line against the token list at `${XDG_CONFIG_HOME:-~/.config}/dotfiles-guard/leakage-tokens.txt` and rename the offending reference. If the hook reports the token list missing while the company-context marker exists, re-run your overlay's `install.sh`.
...
- `_shared/reviewer-blocks/` — shared reviewer contract fragments (tone, anchoring, findings critique, verify-then-draft), spliced into both reviewer SKILL.md files by `reviewer-blocks-gen.sh`
...
- `scripts/check-no-leakage.sh` + `scripts/pre-push.sh` — leakage gates: pre-commit scans the working tree (early warning); pre-push scans the publication set (every outgoing commit's tree and metadata). Token data is materialized outside the repo by an overlay installer — see PROTOCOL.md
...
## Publishing to a team skills repo

The reasoning pipeline (Aristotle → Optimus → Cyrus, Scout/Ranger behind
`code-auditor`, `forge` / `grill-me` / `to-prd` on the front end) can be
exported to a plain skills repository — one that ships only
`skills/<name>/SKILL.md` directories and is consumed by a skills CLI that knows
nothing about `~/.claude/agents`, workflows, `_shared/`, or `evals/`.

```bash
make export-skills TARGET=/path/to/team-skills-repo \
  EXPORT_ARGS='--owner-team my-team --owner-slack "#my-channel"'
make check-export  TARGET=/path/to/team-skills-repo EXPORT_ARGS='…'   # exit 1 on drift
bash scripts/check-portable-refs.sh /path/to/team-skills-repo/skills   # self-containment
```

dotfiles-core stays canonical. `scripts/export-skills.sh` regenerates the nine
managed skill directories in the target (unmanaged skills are untouched) and
maintains a README table between `REASONING PIPELINE TABLE` sentinels. Each
exported skill is self-contained:

- **Agent definitions ride inside the skill** as `skills/<name>/agent.md`. The
  exported SKILL.md gains a *Launching the agent* section: use the registered
  subagent if the host lists it, else launch `general-purpose` primed with
  `agent.md`. `scripts/install-agent.sh` registers the agent for users who
  want the pinned model, `maxTurns`, and memory.
- **Shared assets are duplicated per skill** (`references/agent-turn-cap-warning.md`,
  handoff schemas, `review-heuristics.md`, the `code-auditor-score` workflow)
  so a single-skill install still works.
- **Core-only content is marked at the edit site.** Wrap it in
  `<!-- BEGIN CORE-ONLY -->` … `<!-- END CORE-ONLY -->` (dropped on export) and
  supply a portable alternative inside `<!-- PORTABLE-ONLY … -->` (unwrapped on
  export). Both are HTML comments, so core rendering is unchanged. Path and
  phrase rewrites for `~/.claude`-only references live in the script's
  `_sed_program` table. Templates for the inserted sections live in
  `.claude/_shared/portable/`.

No company-specific strings belong in core: install hints in the exported
text are generic, and owner metadata arrives only via `EXPORT_ARGS`.
