#!/usr/bin/env bash
# lib-seeds.sh — agent memory seeding functions.
# Sourced by install.sh; RESEED is set by the orchestrator.

# Seed agent memory from dotfiles repo seeds directory (seed-once behavior).
# Usage: _seed_agent_memory <dotfiles_dir>
_seed_agent_memory() {
