...
4. **Evaluate the condition:**
   ...
   - `jql` — run via MCP: primary `mcp__claude_ai_Atlassian__searchJiraIssuesUsingJql`<!-- BEGIN OVERLAY-FRAGMENT: obligations-jql-fallback-tool --> <!-- END OVERLAY-FRAGMENT: obligations-jql-fallback-tool -->; check predicate against `result_count`. For company-specific Jira fallback tool name, consult `## Jira fallback` in `~/.claude/overlay-context.md`. If that file is absent, proceed with the primary tool only.
