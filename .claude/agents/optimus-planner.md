...
- **Never leak agent-pipeline vocabulary into snippet comments or identifiers.** Because Cyrus copies your snippets verbatim, any pipeline reference you write ships to source. Keep your `Step N`, assumption/risk IDs (`A9`, `risk R6`), wave/handoff/gate framing, and agent names in the plan's prose and section structure — never inside a code block's comments, identifiers, log strings, or JSX. A bare ticket key in a snippet comment is fine; the step/risk/assumption tokens are not. If a snippet needs to record *why*, phrase it as the underlying constraint, not the plan step that produced it.
